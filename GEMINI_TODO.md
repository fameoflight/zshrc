...
2. Use `git filter-repo` or a similar tool to extract the history of the respective subdirectories (`bin/` for ruby, `bin/python-cli`, `bin/rust-cli`) into the new repositories.
...
- **Action**: Create a new repository. Extract the `bin/rust-cli` directory into it.
- **Files Affected**: `bin/rust-cli/`, `.gitmodules`.
...
- **Action**: Modify the `Makefile` to `cd` into the submodule directories before running language-specific commands (e.g., `cd bin/rust-cli && cargo build`).
- **Files Affected**: `Makefile`.
- **Testing**: Run all major `make` targets (`install`, `mac`, `rust`, `python-models`) to ensure they still work.

### Success Criteria

- [ ] The `bin/python-cli`, `bin/rust-cli`, and Ruby script directories are Git submodules.
