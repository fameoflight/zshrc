...
class GameMode < ScriptBase
  ...
  def script_description
    'Configures display setup for gaming by enabling only the LG OLED monitor and disabling all other displays.'
  end

  def script_arguments
    '[--restore] [--dry-run] [--debug] [--display <index>]'
  end

  def show_examples
    puts 'Examples:'
    puts "  #{script_name}                    # Enable game mode (auto-detect LG OLED)"
  ...
  def show_examples
    puts <<~EXAMPLES
      Examples:
        #{script_name}                    # Enable game mode (auto-detect gaming display)
