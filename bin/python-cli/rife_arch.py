...
def warp(tenInput, tenFlow, tenFlow_div, backwarp_tenGrid):
    ...
    tenFlow = torch.cat([tenFlow[:, 0:1] / tenFlow_div[:, 0:1], tenFlow[:, 1:2] / tenFlow_div[:, 1:2]], 1)
    g = (backwarp_tenGrid + tenFlow).permute(0, 2, 3, 1)
    return F.grid_sample(input=tenInput, grid=g, mode="bilinear", padding_mode="zeros", align_corners=True).to(dtype)
...
class IFBlock(nn.Module):
    ...
    def forward(self, x, flow=None, scale=1):
        ...
        if flow is not None:
            flow = F.interpolate(flow, scale_factor=1.0 / scale, mode="bilinear", align_corners=False, recompute_scale_factor=False) / scale
...
class IFNet(nn.Module):
    """RIFE IFNet for frame interpolation"""
