...
def convert_to_coreml(model, output_name, scale_factor):
    ...
    try:
        # Try neural network format with flexible input shape
        mlmodel = ct.convert(
            traced_model,
        ...
        print("✅ Neural Network conversion successful with flexible input shape!")

    except Exception as e:
        print(f"Neural Network conversion failed: {e}")

        try:
            # Fallback to MLProgram format with flexible input shape
            ...
            print("✅ MLProgram conversion successful with flexible input shape!")
