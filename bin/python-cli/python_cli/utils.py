...
def get_upscaler_dir() -> Path:
    """Get the upscaler-pro-models directory path."""
    workspace = Path.home() / "workspace" / "upscaler-pro-models"
    return workspace


def get_python_executable() -> Path:
    """Get the Python executable from upscaler-pro-models venv."""
    upscaler_dir = get_upscaler_dir()
    venv_python = upscaler_dir / "venv" / "bin" / "python"
...
def get_optimal_inference_params(image_shape, device, scale_factor=4):
    ...
    estimated_memory_gb = (scaled_pixels * 3 * 4 * 4) / (1024**3)  # 3 channels, 4 bytes, 4x safety factor
...
class BaseImageInference:
    ...
    def run_inference(self, input_tensor, tile_size=512, max_workers=None, batch_size=4):
        ...
        if tile_size > 0 and (h > tile_size or w > tile_size):
            print(f'🔳 Processing with batched tiling ({total_pixels:,} pixels, tile={tile_size}x{tile_size})')
    ...
    def inference_tiled(self, input_tensor, tile_size, max_workers=None, batch_size=4):
        ...
        output_size_bytes = torch.tensor(output_shape).prod().item() * 4  # float32 = 4 bytes
        ...
        if output_size_gb > max_reasonable_size:
            print(f'⚠️  Large output detected ({output_size_gb:.1f}GB). Using memory-efficient streaming mode...')
            return self.inference_streaming(input_tensor, tile_size, max_workers, batch_size)

        try:
            output = torch.zeros(output_shape, dtype=input_tensor.dtype, device=self.device)
        ...
        if max_workers is None:
            max_workers = 1 if self.device.type in ['mps', 'cpu'] else min(4, mp.cpu_count())

        print(f'   Processing {total_tiles} tiles with batch size {batch_size} and {max_workers} workers ({tiles_x}x{tiles_y})...')
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                tile = input_tensor[:, :, start_y:end_y, start_x:end_x].to(self.device)
                ...
                if completed % max(1, total_tiles // 20) == 0 or completed == total_tiles:
                    print(f'      Progress: {completed}/{total_tiles} tiles completed ({100*completed/total_tiles:.0f}%)')
    ...
    def inference_streaming(self, input_tensor, tile_size, max_workers=None, batch_size=4):
        ...
        print(f'   🌊 Streaming mode: processing {height}x{width} -> {output_height}x{output_width}')
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                try:
                    ...
                    if "out of memory" in str(e).lower():
                        print(f'   ⚠️  GPU OOM on tile, falling back to CPU for this tile...')
                        tile_cpu = tile.cpu()
                ...
                output_np[out_start_y:out_start_y + out_h, out_start_x:out_start_x + out_w] = tile_np
        ...
        output_tensor = torch.from_numpy(output_np.transpose(2, 0, 1)).unsqueeze(0).float() / 255.0
    ...
    def process_image(self, input_path, output_path, tile_size=None, max_workers=None, batch_size=None):
        ...
        if tile_size is None or max_workers is None or batch_size is None:
            image_shape = (original_size[1], original_size[0])  # (height, width)
            ...
            print_optimization_info(image_shape, tile_size, batch_size, max_workers, self.device)
        else:
            print(f'📊 Using provided parameters: tile={tile_size}, batch={batch_size}, workers={max_workers}')

        # Run inference
        output_tensor = self.run_inference(input_tensor, tile_size, max_workers, batch_size)
