...
def upscale(
    ...
    output_path: Optional[str] = typer.Option(None, help="Output image path (auto-generates if not provided)"),
    model: str = typer.Option("RealESRGAN_x4plus", "--model", "-m", help="Model name to use"),
    ...
    try:
        ...
        if not output_path:
            ...
            output_path = str(input_file.parent / f"{input_file.stem}_upscaled{input_file.suffix}")
        ...
        model_path = model_info.get("coreml_path") or model_info.get("pytorch_path")

        console.print(f"[bold]🤖 Image Upscaling[/bold]")
        console.print(f"Input: {input_path}")
        console.print(f"Output: {output_path}")
        console.print(f"Model: {model}")
        console.print(f"Type: {'CoreML' if model_path.endswith('.mlmodel') else 'PyTorch'}")
        console.print()
        ...
        if model_path.endswith('.mlmodel'):
            ...
            # Fallback to PyTorch inference if needed
            console.print("[yellow]⚠️  Using PyTorch inference (CoreML not available)[/yellow]")
            from python_cli.pytorch_inference import PyTorchInference
            inference = PyTorchInference(model_path)
...
def _list_models():
    ...
    try:
        ...
        if not config.models:
            console.print("[yellow]No models found. Run 'make pytorch-setup' first.[/yellow]")
            return
        ...
        for model_name, model_info in config.models.items():
            model_type = "CoreML" if model_info.get("coreml_path") and Path(model_info["coreml_path"]).exists() else "PyTorch"
