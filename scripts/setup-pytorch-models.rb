...
class SetupPytorchModels
  attr_reader :models_dir, :pytorch_dir, :apple_silicon_dir
  ...
  def initialize
    ...
    FileUtils.mkdir_p([@models_dir, @pytorch_dir, @apple_silicon_dir])
  end
...
def install_dependencies
  ...
  result = system("#{@python_cmd} -c \"import coremltools; print('CoreML tools available')\"")
  if result
  ...
  def download_model(name, config)
    model_path = File.join(@pytorch_dir, config['filename'])
